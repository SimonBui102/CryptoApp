          ...
          <tbody>
            {cryptoData.map((data) => {
              return (
                <tr
                  key={data.id}
                  className="text-center text-base border-b border-gray-500 hover:bg-gray-800 last:border-b-0"
                >
                  <td className="py-4 flex items-cente uppercase">
                    <button className="outline-0 border-0 bg-none cursor-pointer">
                      <svg
                        className="w-[1.5rem]  ml-1.5  fill-gray-400 hover:fill-yellow-300"
                        width="20"
                        height="20"
                        viewBox="0 0 30 30"
                        fill="yellow"
                    ...
                    <img
                      className="w-[1.4rem] h-[1.4rem] mx-1.5"
                      src={data.image}
                      alt={data.name}
                    />
                  ...
                  <td className= {data.price_change_percentage_1h_in_currency > 0 ? 'text-green-400 py-4':  'text-red-400 py-4'}>
                    {Number(data.price_change_percentage_1h_in_currency).toFixed(2)}%
                  </td>
                  <td className= {data.price_change_percentage_24h_in_currency > 0 ? 'text-green-400 py-4':  'text-red-400 py-4'}>
                    {Number(data.price_change_percentage_24h_in_currency).toFixed(2)}%
                  </td>
                  <td className= {data.price_change_percentage_7d_in_currency > 0 ? 'text-green-400 py-4':  'text-red-400 py-4'}>
